...
`itorchestra-root` (registered by Terraform in 0.12.1) recurses this directory. Every
...
- Once a wave is confirmed to diff cleanly against live, auto-sync (`prune` + `selfHeal`) is
  enabled for it in a later step (0.12.6 drift detection).
...
|  -8  | `platform-storage-class`     | `k8s/cluster/longhorn` (SC only)    | done (adopted; StorageClass only; Longhorn chart adopted later) |
|  -7  | `ingress-nginx`              | helm 4.11.3 + repo values (multi-src) | done (adopted clean; auto-sync selfHeal+prune; webhook caBundle ignored) |
|  -7  | MetalLB pool                 | n/a                                 | **deferred in dev** - the pool is auto-detected from the VM NIC at runtime (`metallb/install.sh`); the repo `ippool.dev.yaml` is only an example and does NOT match the live `10.178.95.240-250` pool. GitOps-ifying it would break the LB IP. Revisit with a static per-env pool in 0.12.5. |
...
|   5  | `keycloak`                   | `k8s/keycloak` (manifests + Job)    | created (manual; diff review pending; db-init Job=Sync hook; KC_HOSTNAME ignored) |
